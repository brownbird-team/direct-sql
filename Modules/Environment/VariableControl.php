...
class VariableControl {
    ...
    public function pop_query_vars() {
        unset($this->query_stack[--$this->next_query]);
    }

    // Dodaj globalnu varijablu
    public function set_global_var(string $name, $value, int $line) {
        $this->global_vars[$name] = $value;
    }

    // Pobriši globalnu varijablu
    public function delete_global_var(string $name, int $line) {
    ...
    public function get_global_var(string $name, string $type, int $line) {

        if (!isset($this->global_vars[$name]))
            throw new PandaExecuteError('Trying to access value of non-existent GLOBAL variable $'. $name, $line);

        $this->check_type($this->global_vars[$name], $type);
