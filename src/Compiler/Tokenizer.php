...
class Tokenizer {
    ...
    public function tokenize($input) {
        ...
        while ($pointer < $input_length) {
            ...
            if (substr($input, $pointer, 2) == '%}') {
                ...
                if ($position == HTML_STRING) {
                    throw new PandaParseError('found extra %}', $line_count, $this->page);
                }

                if ($position == SQL_QUERY_STRING) {
                    if (strlen($buffer) > 0)
                        $tokens[] = new Tokens\TokenTypeSqlString($buffer, $line_count, $this->page);

                    $tokens[] = new Tokens\TokenTypeSqlEnd($line_count, $this->page);
                }

                $tokens[] = new Tokens\TokenTypeCommandEnd($line_count, $this->page);
            ...
            if ($position == HTML_STRING) {
                if (substr($input, $pointer, 2) == '{%') {
                    ...
                    if (strlen($buffer) > 0) {
                        $tokens[] = new Tokens\TokenTypeHtmlString($buffer, $line_count, $this->page);
                    }
            ...
            if ($position == COMMAND_SPACE) {
                ...
                if (preg_match(IS_PARENTHESIS, $ch)) {
                    $tokens[] = new Tokens\TokenTypeParenthesis($ch, $line_count, $this->page);
                    $pointer++;
                    continue;
                }
                ...
                if (preg_match(IS_NAME_START, $ch)) {
                    ...
                    $tokens[] = new Tokens\TokenTypeName($buffer, $line_count, $this->page);
                    ...
                    if ($buffer == 'query') {
                        ...
                        $buffer = '';
                        $tokens[] = new Tokens\TokenTypeSqlStart($line_count, $this->page);
                    }
                ...
                if (preg_match(IS_NUMBER, $ch)) {
                    ...
                    $tokens[] = new Tokens\TokenTypeNumber($buffer, $line_count, $this->page);
                ...
                if ($ch == QUOTE_CHARACTER) {
                    ...
                    while ($ch !== QUOTE_CHARACTER) {
                        ...
                        if ($pointer == $input_length - 1)
                            throw new PandaParseError('Failed to find matching quote for string', $start_line, $this->page);
                        ...
                        if ($ch == ESCAPE_CHARACTER) {
                            ...
                            throw new PandaParseError('Character "'. $ch .'" cannot be escaped', $line_count, $this->page);
                        }
                    ...
                    $pointer++;
                    $tokens[] = new Tokens\TokenTypeString($buffer, $start_line, $this->page);
            ...
            if ($ch == VARIABLE_START_CHARACTER) {
                ...
                if (preg_match(IS_NAME_START, $input[$name_start])) {
                    ...
                    $tokens[] = new Tokens\TokenTypeVariable($buffer, $var_type, $line_count, $this->page);
                ...
                if ($position == COMMAND_SPACE) {
                    throw new PandaParseError('Variable name cannot start with character "'. $input[$name_start] .'"', $line_count, $this->page);
                }
            }
            ...
            throw new PandaParseError('Unexpected character "'. $ch .'" found in command mode', $line_count, $this->page);
        }

        if ($position !== HTML_STRING) {
            throw new PandaParseError('Error: Failed to find matching %}', $line_count, $this->page);
        }
        ...
        $tokens[] = new Tokens\TokenTypeProgramEnd($line_count, $this->page);
